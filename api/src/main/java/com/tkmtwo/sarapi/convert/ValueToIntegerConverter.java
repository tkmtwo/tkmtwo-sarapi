 ...
 *
 */
public final class ValueToIntegerConverter
  implements Converter<Value, Integer> {
  
  @Override
  public Integer convert(Value v) {
    if (v == null || v.getValue() == null || v.getDataType() == DataType.NULL) {
      return null;
    }
    
    if (!v.getDataType().equals(DataType.INTEGER)
        && !v.getDataType().equals(DataType.ENUM)) {
      throw new InvalidValueAccessException("Value is neither DataType.INTEGER nor DataType.ENUM");
    }
